...
Full reference for every script in this repo: options, behavior, cautions, and
the Music.app AppleScript quirks they are built around. For the short version,
see [README.md](README.md).

Every script drives `Music.app` directly through AppleScript. Nothing touches
...
Every script also takes `--progress`. It is off by default and changes nothing
about the human output; with it on, the script `log`s one machine-readable
line per event to stderr, tab-separated: `@@ begin`, then one `@@ item` per
track or playlist as it is handled (`i`, `of`, `kind`, `playlist`, `artist`,
`title`, and `id` -- the track's persistent ID -- where the script knows it),
then `@@ end`. Kinds are things like `added`, `downloaded`, `pending`,
`unavailable`, `queued`, `created`, `removed`, `backed-up`, `restored`. A later
item with the same `id` updates the earlier row in the app rather than adding
one, which is how `watch-downloads` turns a pending row into a downloaded one.
The app passes the flag on every run and turns the lines into its progress bar
and live feed. Emitting costs no extra per-track Apple events: the
scripts that already loop per track report from that loop, and the bulk paths
(`build-genres`) read `name` and `artist` of a playlist's tracks in one call
after filling it.
...
```sh
./build-genres.applescript --dry-run     # preview the plan
./build-genres.applescript               # first build, or file only what is new
./build-genres.applescript --replace     # delete the folder and rebuild from scratch
```

| Option | Meaning |
| --- | --- |
| `--dry-run` | Print the plan and any unmapped genres; create nothing. |
| `--replace` | Delete an existing folder of the same name first and rebuild everything. |
| `--folder NAME` | Folder to create (default `genres`). |
| `--min-tracks N` | Skip genres with fewer than N tracks (default 1). |

**Two modes.** With no folder yet, or with `--replace`, it does the full build.
With the folder already present and no `--replace`, it is **incremental**: only
tracks not yet in any genre playlist are touched. A track whose genre already
has a playlist is appended to it; a genre with no playlist yet gets one created
in full, inside its parent folder (creating the parent if this is the first
genre of its group). Nothing existing is renamed, moved or removed, and
`--min-tracks` only gates the *new* playlists.

Appending rows to an existing synced playlist is still a modification, and
whether iCloud keeps those rows is not established (row *deletions* were
reverted; additions have not been tested). Check with `whats-new` a day later;
if the rows vanished, `--replace` remains the sure path.
...
./whats-new.applescript --json       # also append one machine-readable summary line
```

`--json` appends a single line such as
`{"library":2494,"filed":2494,"playlists":84,"folders":13,"unfiled":0}` after
the report, for programs (the app's overview reads it). The human output is
unchanged.
...
or run `watch-downloads` to be told each track as it lands.
Tracks with cloud status `no longer available` are counted separately, since
they can never download.

### `watch-downloads.applescript` — watch queued downloads land (read-only)

```sh
./watch-downloads.applescript                  # check every 30 s until nothing is pending
./watch-downloads.applescript --interval 10    # check more often
./watch-downloads.applescript --once           # one check after the interval, then exit
```

The other half of the downloads pipeline: `download-genres` queues, this
watches. Music works through a queue in the background and reports nothing, so
the watcher polls for the one signal there is, a track gaining a `location`,
and names the song. On start it walks the genre playlists once, exactly as
`download-report` does (one `location` read per track), and remembers every
track without a local file that is not `no longer available`. Then, every
`--interval` seconds, for each playlist that still has pending tracks it does
one bulk read of the playlist's persistent IDs and a `location` read only for
the tracks still pending; a track once seen downloaded is never asked again.
Playlists are re-found by persistent ID on every pass. It exits when nothing is
pending, and killing it any time is safe: it writes nothing. It contains no
`download` command.
...
./dedupe-playlists.applescript --dry-run --json   # plus one summary line for programs
```

`--json` appends `{"library":…,"playlists":…,"rows":…,"distinct":…,"duplicates":…,"touched":…}`
as the last line; the app's overview reads the duplicate count from it.
...
./restore-playlists.applescript --dry-run backups/playlists-20260905-141540   # list, create nothing
```

`--dry-run` lists each playlist the backup holds and how many track IDs it
has, without talking to Music at all. It cannot say how many of those tracks
are still in the library; only the real run finds that out.
